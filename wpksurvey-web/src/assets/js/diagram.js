function diagramJsInit(uri, company) {
...
surveyDiagram = async function() {
  ...
  let svRes = await fetch(baseUrl+'/company/'+companyId+'/surveys');
  if (svRes.ok) {
...
interactionDiagram = async function () {
  ...
  let clRes = await fetch(baseUrl + '/company/'+ companyId +'/clients');
  ...
  if (clRes.ok) {
    ...
    if (data.statusCode==='SUCCESS') {
      ...
      for (let client of clients) {
        for (let account of client.clientSurveys) {
  ...
  if (published.length > 1) {
    title = 'Publier à '+ published.length +' contacts';
  } else {
    title = 'Publier à '+ published.length +' contact';
  }
        ...
        if (questions.length!==0) {
          ...
          for (let j=0; j<questions.length; j++) {
            ...
            if (question.surveyQuestionStatus === 'active' && question.surveyQuestionFieldType!=='input') {
              ...
              ditem.style.cssText = "; width: 300px; background: #ffffff; box-shadow: 0px 1px 4px #cec6c6; margin: 10px 0; padding: 10px;"
