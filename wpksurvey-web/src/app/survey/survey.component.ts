...
export class SurveyComponent implements OnInit {
  ...
  stepManager(step: number) {
    ...
    this.surveyServices.getClientResponsesByToken(this.token, data => {
  ...
  onSubmitResponseClient(responseForm: NgForm) {
    ...
    switch (fieldsType) {
      ...
      case 'checkbox':
        this.checkboxManager(responseForm);
        break;
  ...
  private checkboxManager(responseForm: NgForm) {
    this.surveyServices.removeClientResponse(this.token, this.selectedQuestion?.surveyQuestionId, ()=> {
      for (let i = 0; i < this.suggestedResponse.length; i++) {
        let value: string = 'false';
  ...
  sendResponseClient(questionId: number | undefined, responseId: number | undefined, details: string) {
